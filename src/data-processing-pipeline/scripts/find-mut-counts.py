...
def find_mut_count(file, out_dir, ref_seq=None, ref_labels=None, d=5):
    """ Finds the counts of mutant alleles at each site"""
    df                = pd.read_csv(file)
    data              = get_data(file)
    sequences         = data['sequences']
    ref_sites         = data['ref_sites']
    mutant_sites_samp = ref_sites   #just needed for indexing
    dates             = data['dates']
    ...
    np.savez_compressed(f, mutant_sites=mutant_sites_samp, mut_counts=mut_counts, ref_sites=ref_sites)
    f.close()
...
def main(args):
    ...
    find_mut_count(in_file, out_dir, ref_seq=ref_nucs, ref_labels=index)    
