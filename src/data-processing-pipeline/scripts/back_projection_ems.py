...
def back_projection_fit(read_dir, write_dir, incubation_file, smoothing_file):
...
# edited for sub-directories inside directories

def back_projection_fit_sub(read_dir, write_dir, incubation_file, smoothing_file):
    ...
    for root, dirs, files in os.walk(read_dir):
        for dir in dirs:
            ...
            output_dir = os.path.join(write_dir, os.path.relpath(sub_dir, read_dir))

            os.makedirs(output_dir, exist_ok=True)

            observation_files = [os.path.join(sub_dir, f) for f in os.listdir(sub_dir) if f.endswith(".csv")]
            
            for file in observation_files:
                
                output_file = os.path.join(output_dir, os.path.basename(file.split('.')[0] + '_rates.csv'))
                df_inc = pd.read_csv(incubation_file, memory_map=True)
                df_smooth = pd.read_csv(smoothing_file, memory_map=True)
                df_obs = pd.read_csv(file, memory_map=True)
                ...
                if np.min(df_obs['count'])<0:
                    print('Counts file contains negative values!')

                time = df_lam['time'].values
                for t in time:
                    n_rows = len(df_lam[df_lam['time']==t])
                    if n_rows!=1:
                        print('Got %d observation rows with time=%d!' % (n_rows, t))
...
def parse_args():
    ...
    parser.add_argument('-read_dir', help='read directory')
...
def main():
    ...
    if not os.path.exists(read_dir):
        raise Exception('directory does not exist ({0}).'.format(read_dir))
    if not os.path.exists(write_dir):
        raise Exception('directory does not exist ({0}).'.format(write_dir))
        
    back_projection_fit(read_dir, write_dir, smoothing_file, incubation_file)
