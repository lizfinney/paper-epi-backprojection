...
def main(args):
    ...
    for file in sorted(os.listdir(directory)):
        ...
        #print(filename)    
        # Skip .DS_Store file, skip other directory file
    ...
    alleles_temp  = [str(i) for i in alleles_temp]
    allele_number = []
    ref_poly      = []
    ...
    for sim in range(len(filepaths)):
        ...
        #allele_number  = [int(i) for i in allele_number]
        ...
        for i in range(len(mutant_sites)):
            #print(counts[i])
            #print(f"allele_number: {allele_number}")
            #print(f"ref_sites shape: {np.array(ref_sites).shape}")
