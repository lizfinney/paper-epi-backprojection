...
// Retrieve population sizes from a file

void getN(FILE *input, std::vector<double> &N) {
...
void printCovariance(FILE* output, double cov[], int L, int T){
    
    for (int k=0;k<T;k++) {

        for (int i=0;i<L;i++) {
    
            fprintf(output,"%.4e",cov[(k * T) + (i * L)]);
            for (int j=1;j<L;j++) fprintf(output," %.4e",cov[(k * T) + (i * L) + j]);
            fprintf(output,"\n");
