...
//    double alpha = 0.05;
//
//    for (int i=0;i<sequences[0].size();i++) { for (int a=0;a<q-1;a++) {
//
//        for (int j=i+39;j<sequences[0].size();j++) { for (int b=0;b<q-1;b++) {
//
//            int ax = (i * (q-1)) + a;
//            int bx = (j * (q-1)) + b;
//
//            p2[(ax * L) + bx] = ((1 - alpha) * p2[(ax * L) + bx]) + (alpha * p1[ax] * p1[bx]);
//            p2[(bx * L) + ax] = p2[(ax * L) + bx];
//
//        } }
//
//    } }
...
//            for (int b=0;b<a;b++) {
//
//                fluxIn  += p1[(i * q) + b] * muMatrix[b][a];
//                fluxOut += p1[(i * q) + a] * muMatrix[a][b];
//
//            }
//            for (int b=a+1;b<q;b++) {
//
...
//    int L = ((int) sequences[0][0].size()) * q;     // sequence length (i.e. number of tracked alleles)
    ...
    int nInterp =  0;   // track number of interpolations
    int dtMax   = 20;   // max spacing between generations before interpolating
    ...
    for (int k=1;k<sequences.size();k++) {
        ...
        // If time points are too far apart, add interpolated time points
        
        if (times[k]-times[k-1]>dtMax) {
        
            int    nSteps = ((int) (times[k] - times[k-1])/dtMax) + 1;
            double dt     = 1 / nSteps;
            
            for (double n=0;n<nSteps;n++) {
    
                interpolateFrequencies(lastp1, lastp2, p1, p2, ((n + 1.0) / nSteps) - (0.5 * dt), xp1, xp2);  // Stratonovich
                //interpolateFrequencies(lastp1, lastp2, p1, p2, ((n + 1.0) / nSteps) - dt, xp1, xp2);          // Ito
                
                updateCovariance((times[k] - times[k-1])/nSteps, xp1, xp2, totalCov);
                updateMu((times[k] - times[k-1])/nSteps, muMatrix, xp1, totalMu);
            ...
            interpolateFrequencies(lastp1, lastp2, p1, p2, 0.5, xp1, xp2);  // Stratonovich convention
            //interpolateFrequencies(lastp1, lastp2, p1, p2, 0, xp1, xp2);  // Ito convention
...
//    for (int a=0;a<L;a++) totalCov[(a * L) + a] += gammaN;     // standard regularization + diagonal part of gauged state regularization
...
int run(RunParameters &r) {
    ...
    Vector counts;              // counts for each sequence at each time point, divided by total number of sequences at that time
...
//    if (r.q>1) expandSequences(r.q, sequences);
