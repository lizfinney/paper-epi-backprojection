...
//    double alpha = 0.5;
//    for (int a=0;a<p1.size();a++) p1[a] = ((1 - alpha) * p1[a]) + (alpha / 20.);
//    for (int a=0;a<p2.size();a++) p2[a] = ((1 - alpha) * p2[a]) + (alpha / 400.);
...
//    int L = ((int) sequences[0][0].size()) * q;     // sequence length (i.e. number of tracked alleles)
    ...
    double dtMax   = 20;   // max spacing between generations before interpolating
    ...
    for (int k=1;k<sequences.size();k++) {
        ...
        if (times[k]-times[k-1]>dtMax) {
            ...
            double nSteps = ((int) (times[k] - times[k-1])/dtMax) + 1;
            double dt     = 1.0 / nSteps;

            for (double n=0;n<nSteps;n++) {

                interpolateFrequencies(lastp1, lastp2, p1, p2, ((n + 1.0) / nSteps) - (0.5 * dt), xp1, xp2);  // Stratonovich convention
                //interpolateFrequencies(lastp1, lastp2, p1, p2, ((n + 1.0) / nSteps) - dt, xp1, xp2);        // Ito convention

                //--- debug
                if (useDebug) {
                    printf("%d\t%d\t%d\t%d\t%d\t%.4f\t%.4f\t%.2f\n",(int)times[k-1],(int)times[k],(int)(times[k]-times[k-1]),(int)n,(int)nSteps,dt,n*dt,(times[k]-times[k-1])*dt);
                }
                //---

                updateCovariance((times[k] - times[k-1]) * dt, xp1, xp2, totalCov);
                updateMu((times[k] - times[k-1]) * dt, muMatrix, xp1, totalMu);
