...
//    int L = ((int) sequences[0][0].size()) * q;     // sequence length (i.e. number of tracked alleles)
    ...
    double dtMax   = 100;   // max spacing between generations before interpolating
...
//
//            double nSteps = round((times[k] - times[k-1])/dtMax) + 1;
//            double dt     = 1.0 / nSteps;
//
//            for (double n=0;n<nSteps;n++) {
//
//                //interpolateFrequencies(lastp1, lastp2, p1, p2, (n + 0.5) * dt, xp1, xp2);  // Stratonovich convention
//                interpolateFrequencies(lastp1, lastp2, p1, p2, n * dt, xp1, xp2);        // Ito convention
//
