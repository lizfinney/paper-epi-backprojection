...
#include "inf-cut.h" // inference declarations
...
void updateCovarianceIntegrate(double dg, // time step
                               double alpha, // fractional time step (0 - 1 --> t0 - t1)
    ...
    for (int a=0;a<L;a++) {
        ...
        totalCov[(a * L) + a] += dg * alpha * ( (6 * p1_0[a] * (1 - p1_0[a])) - (3 * alpha * (p1_0[a] - p1_1[a]) * (1 - (2 * p1_0[a])))
                                                 - (2 * alpha * alpha * (p1_0[a] - p1_1[a]) * (p1_0[a] - p1_1[a])) ) / 6;
        
        for (int b=a+1;b<L;b++) {
            
            double dCov1 = dg * alpha * alpha * ( (3 * (p2_1[(a * L) + b] - p2_0[(a * L) + b] + (2 * p1_0[a] * p1_0[b]) - (p1_0[a] * p1_1[b]) - (p1_1[a] * p1_0[b])))
                                                  - (2 * alpha * (p1_1[a] - p1_0[a]) * (p1_1[b] - p1_0[b])) ) / 6;
            double dCov2 = dg * alpha * (p2_0[(a * L) + b] - p1_0[a] * p1_0[b]);
    ...
    for (int i=0;i<L;i++) {
        ...
        for (int a=0;a<q;a++) {
            ...
            for (int b=0;b<a;b++) {
                ...
                fluxIn  += 0.5 * alpha * ( (2 * p1_0[(i * q) + b]) + (alpha * (p1_1[(i * q) + b] - p1_0[(i * q) + b])) ) * muMatrix[b][a];
                fluxOut += 0.5 * alpha * ( (2 * p1_0[(i * q) + a]) + (alpha * (p1_1[(i * q) + a] - p1_0[(i * q) + a])) ) * muMatrix[a][b];
...
void processStandard(const IntVVector &sequences, // vector of sequence vectors
    ...
    for (int k=1;k<sequences.size();k++) {
        ...
        if (times[k-1]>TMAX) break;
    
        computeAlleleFrequencies(sequences[k], counts[k], q, p1, p2);
        
        if (times[k]>TMAX) {
        
            double dt_total = times[k] - times[k-1];
            double dt_trunc = TMAX - times[k-1];
            double alpha = dt_trunc/dt_total;
            
            updateCovarianceIntegrate(dt_total, alpha, lastp1, lastp2, p1, p2, totalCov);
            updateMuIntegrate(dt_total, alpha, muMatrix, lastp1, p1, totalMu);
            
            interpolateFrequencies(lastp1, lastp2, p1, p2, alpha, xp1, xp2);
            
            for (int a=0;a<L;a++) dx[a] += xp1[a];
        
        }
        
        else {
        
            updateCovarianceIntegrate(times[k] - times[k-1], 1, lastp1, lastp2, p1, p2, totalCov);
            updateMuIntegrate(times[k] - times[k-1], 1, muMatrix, lastp1, p1, totalMu);
