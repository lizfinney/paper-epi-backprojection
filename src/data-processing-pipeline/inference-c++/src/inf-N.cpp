...
#include "inf-N.h"    // inference declarations
...
void processStandard(const IntVVector &sequences, // vector of sequence vectors
                     ...
                     const std::vector<double> &N, // population sizes
    ...
    for (int k=1;k<sequences.size();k++) {
        ...
        double dg   = times[k] - times[k-1];
        double Navg = (N[k] + N[k-1])/2;
    
        computeAlleleFrequencies(sequences[k], counts[k], q, p1, p2);
        
        for (int a=0;a<L;a++) dx[a] += Navg * (p1[a] - lastp1[a]);
        
        updateCovarianceIntegrate(dg * Navg, lastp1, lastp2, p1, p2, totalCov);
        updateMuIntegrate(dg * Navg, muMatrix, lastp1, p1, totalMu);
        
        if (k!=sequences.size()-1) { lastp1 = p1; lastp2 = p2; }
...
void regularizeCovariance(const IntVVector &sequences, // vector of sequence vectors
    ...
    for (int a=0;a<L;a++) totalCov[(a * L) + a] += gamma; // standard regularization (no gauged state)
...
int run(RunParameters &r) {
    ...
    std::vector<double> N;  // list of population sizes
    
    if (r.useN) {
    
        if (FILE *Nin = fopen(r.getNInfile().c_str(),"r")) { getN(Nin, N); fclose(Nin); }
        else { printf("Problem retrieving data from file %s! File may not exist or cannot be opened.\n", r.getNInfile().c_str()); return EXIT_FAILURE; }
        
    }
    else {
        
        for (int i=0;i<times.size();i++) N.push_back(r.N);
    ...
    double gamma     = r.gamma;                                 // regularization strength
    ...
    else                 processStandard(sequences, counts, times, N, muMatrix, r.q, totalCov, dx);
    ...
    regularizeCovariance(sequences, r.q, gamma, r.useCovariance, totalCov);
