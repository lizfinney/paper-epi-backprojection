...
void processStandard(const IntVVector &sequences, // vector of sequence vectors
    ...
    for (int i=0;i<(L * L);i++) {totalCov[i] += lastp2[i]};
    ...
    for (int k=1;k<sequences.size();k++) {
        ...
        for (int i=0;i<(L * L);i++) {totalCov[(k * L * L) + i] += p2[i]}; 
...
int run(RunParameters &r) {
    ...
    double *totalCov = new double[T*L*L];                         // accumulated allele covariance matrix
    ...
    if (r.saveCovariance) {
        if (FILE *dataout = fopen(r.getCovarianceOutfile().c_str(),"w")) { printCovariance(dataout, totalCov, L, T); fclose(dataout); }
