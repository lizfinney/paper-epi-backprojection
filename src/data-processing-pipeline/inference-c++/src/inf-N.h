...
public:
    ...
    std::string NInfile;               // input file for population sizes
    ...
    double N;               // (constant) population size
    double mu;              // mutation rate per generation
    int q;                  // number of states for each allele
    
    bool useN;              // if true, read population sizes from file
    ...
    std::string getNInfile()                     { return (directory+"/"+NInfile);     }
...
void processStandard(const IntVVector &sequences, const Vector &counts, const std::vector<double> &times, const std::vector<double> &N, const Vector &muMatrix, int q, double totalCov[], double dx[]);
