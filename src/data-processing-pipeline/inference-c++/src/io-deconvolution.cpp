...
#include "inf-deconvolution.h"    // inference declarations
#include "io-deconvolution.h"     // input/output
...
void printSelectionCoefficients(FILE *output, const std::vector<double> &s) {
    ...
    for (int i=0;i<s.size();i++) fprintf(output,"%.8e\n",s[i]);
    fflush(output);
...
void printCovariance(FILE* output, double cov[], int L){
    ...
    for (int i=0;i<L;i++) {
        ...
        fprintf(output,"%.8e",cov[(i * L)]);
        for (int j=1;j<L;j++) fprintf(output," %.8e",cov[(i * L) + j]);
        fprintf(output,"\n");
...
void printCovariance(FILE* output, const std::vector<double> &cov, int L){
...
void printCovariance(FILE* output, const Vector &cov, int L){
    
    for (int i=0;i<L;i++) {
    
        fprintf(output,"%.8e",cov[i][0]);
        for (int j=1;j<L;j++) fprintf(output," %.8e",cov[i][j]);
        fprintf(output,"\n");
...
//void printCovarianceCompressed(FILE* output, const std::vector<double> &cov, int L){
//
//    for (int i=0;i<L;i++) {
//
//        fprintf(output,"%.8e",cov[hindex(i, i, L)]);
//        for (int j=1;j<i;j++) fprintf(output," %.8e",cov[hindex(j, i, L)]);
//        for (int j=i;j<L;j++) fprintf(output," %.8e",cov[hindex(i, j, L)]);
//        fprintf(output,"\n");
//
//    }
//    fflush(output);
...
void printNumerator(FILE* output, const std::vector<double> &dx, int L){
